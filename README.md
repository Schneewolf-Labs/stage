...
egirl is untouched: Stage is a client of egirl's HTTP API (`POST /chat` streaming, plus the
session and `/info` endpoints the console reads), and optionally of a Wald registry to find it.
...
[Wald](https://github.com/Schneewolf-Labs/Wald) registry supply the address: set `[wald] url =
"http://wald.internal:8090"` (Wald's REST API, not its MCP port) and `egirl = "kira"` on the
talent (the slug the instance registered under). The instance registers itself when its egirl
config enables `[peer_discovery]` with a `self_url`; a row seeded by hand as `protocol = "mcp"`
is refused. `serve` asks Wald (`GET /agents/{slug}`) at startup and accepts only an agent with
`protocol = "egirl-peer/1"`, `status = "active"` and an `endpoint_url`; otherwise it refuses to
start and says which. If that egirl later stops answering, the next failed turn asks Wald again,
so an instance that moved is picked up without a restart. A pinned `egirl_url` wins over `egirl`,
and `doctor` shows the resolved URL or why resolution failed. Tokens still come from
`egirl_token`: Wald stores where an instance is, not how to authenticate to it.
